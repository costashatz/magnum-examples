...
void main(){
	...
	for(uint i = 0; i < maxLights; ++i) col += calculatePointLight(pointLights[i]);
	...
	col = (diff + spec) * col + clamp(material.emissivity, 0, 1) * material.diffuseColor;
	...
	vec4 res = alpha * vec4(vec3(col), 1);
